...
# Copyright (C) 2023  Vinzenz Hassert <thezeanon@gmail.com>
...
class FirmwareRetraction:
    ...
    def cmd_SET_RETRACTION(self, gcmd):
        if not self.is_retracted:        # Only execute command when unretracted
            ...
            if self.verbose:
                gcmd.respond_info('Printer in retract state.'
    ...
    def cmd_CLEAR_RETRACTION(self, gcmd):
        if self.is_retracted:
            ...
            if self.verbose:
                gcmd.respond_info('Retraction, including '
                                  'SET_RETRACTION command '
                                  'queue, was cleared and '
                                  'reset to config '
                                  'values. zhop is undone '
                                  'on next move.')
        else:
            if self.verbose:
    ...
    cmd_FIRMWARE_RETRACTION_START_PRINT_help = (
        "Tells the Retraction a print is started"
    )

    def cmd_FIRMWARE_RETRACTION_START_PRINT(self, gcmd):
        self._evaluate_retraction()

    cmd_FIRMWARE_RETRACTION_END_PRINT_help = (
        "Tells the Retraction a print has ended"
    )

    def cmd_FIRMWARE_RETRACTION_END_PRINT(self, gcmd):
        self._evaluate_retraction()
    ...
    def cmd_G10(self, gcmd):
        ...
        if 'xyz' not in homing_status:  # If printer is not homed, ignore command
            ...
            if self.verbose:
                gcmd.respond_info('Extruder temperature too low. '
                                  'Command ignored!')
        elif self.retract_length == 0.0:        # Check if FW retraction enabled
            if self.verbose:
                gcmd.respond_info('Retraction length zero. '
            ...
            ).format(self.acc_vel_state[0],
                     self.acc_vel_state[1],
                     self.acc_vel_state[2],
                     self.acc_vel_state[3])
    ...
    def cmd_G11(self, gcmd):
        ...
        if self.retract_length == 0.0:          # Check if FW retraction enabled
            if self.verbose:
                gcmd.respond_info('Retraction length cero. '
                                  'Firmware retraction disabled. '
                                  'Command ignored!')
            ...
            if not self.extruder.heater.can_extrude:
                self._execute_clear_retraction()
                if self.verbose:
                    gcmd.respond_info('Extruder temperature low. '
                                      'Retraction cleared without retract '
                                      'move. zhop will be '
                ...
                if self.z_hop_height > 0.0 and self.ramp_move:
                    ...
                    unretract_gcode += (
                        ...
                        "G1 Z-{:.5f} F{}\n"
                             ...
                             int(RETRACTION_MOVE_SPEED_FRACTION
                                 * self.max_vel * 60))
                ...
                ).format(self.unretract_length,
                         int(self.unretract_speed * 60),
                         self.acc_vel_state[0],
                         self.acc_vel_state[1],
                         self.acc_vel_state[2],
                         self.acc_vel_state[3])

                self.gcode.run_script_from_command(unretract_gcode)
                self.is_retracted = False          # Set the flag to unretracted
                self.acc_vel_state = []         # Reset acc and vel setting list
    ...
    def _handle_ready(self):
        ...
        gcode.register_command('SET_RETRACTION',
                               self.cmd_SET_RETRACTION,
                               desc=self.cmd_SET_RETRACTION_help
                               )
        gcode.register_command('GET_RETRACTION',
                               self.cmd_GET_RETRACTION,
                               desc=self.cmd_GET_RETRACTION_help
                               )
        gcode.register_command('CLEAR_RETRACTION',
        ...
        self.printer.register_event_handler("stepper_enable:motor_off",
                                            self._evaluate_retraction)
        self.printer.register_event_handler("stepper_enable:axes_off",
                                            self._evaluate_retraction)
        ...
        if self.config_ref.has_section('virtual_sdcard'):
            ...
            self.printer.register_event_handler(
                "print_stats:cancelled_printing", self._reset_pause_flag)
            self.printer.register_event_handler(
    ...
    def _toggle_gcode_comms(self,
        ...
        if not toggle_state:
            ...
            self.gcode.register_command(new_cmd_name,
                                        prev_cmd,
                                        desc=pdesc)
            self.gcode.register_command(old_cmd_name,
                                        new_cmd_func,
                                        desc=new_cmd_desc)
